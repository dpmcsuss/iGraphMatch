...
In `iGraphMatch` package, `sample_correlated_gnp_pair` is a useful function to generate graph pairs from Correlated Erdős-Rényi Random Graphs model. `sample_correlated_rdpg_pair` function is for sampling a pair of graphs from random dot product graphs model.
...
```{r}
set.seed(123)
match_bari <- gm(g1, g2, seeds, method = "indefinite", start = start_bari)
match_rds <- gm(g1, g2, seeds, method = "indefinite", start = start_rds)
match_convex <- gm(g1, g2, method = "indefinite", seeds, start = start_convex)
...
```{r }
set.seed(123)
match_nss_bari <- gm(g1, g2, seeds, method = "indefinite", start = "bari")
match_nss_rds <- gm(g1, g2, seeds, method = "indefinite", start = "rds")
match_nss_convex <- gm(g1, g2, seeds, method = "indefinite", start = "convex")
```
...
match_bari <- gm(g1, g2, seeds, method = "indefinite", start = start_bari)
match_rds <- gm(g1, g2, seeds, method = "indefinite", start = start_rds)
match_convex <- gm(g1, g2, seeds, method = "indefinite", start = start_convex)
...
### Simulations
Now implement algorithm of finding core vertices in R with `iGraphMatch` package. First we sample a pair of graphs with 50 vertices from the correlated stochastic block model and let 5 out of the total vertices to be junk vertices using function `sample_correlated_sbm_pair`. We also assume the first 10 vertices to be seeds. Then apply the FAQ algorithm to match two graphs.
```{r}
set.seed(5)
pm <- cbind( c(.3, .5), c(.5, .7) )
sbm_pair <- sample_correlated_sbm_pair(n = 50, pref.matrix = pm, corr = 0.5,
                                              block.sizes = c(15,35), 
                                              core.block.sizes = c(15,30))
g1 <- sbm_pair$graph1
g2 <- sbm_pair$graph2
...
match <- gm(g1, g2, seeds, method = "indefinite", start = "rds")
```
