...
test_that("input scheme check", {
  expect_error(center_graph(A, scheme = c(-1, 1, 1)), 
               "scheme must be either 'center', 'naive', a positive integer, or a pair of scalars.")
  expect_error(center_graph(A, scheme = "other scheme"), 
               "scheme must be either 'center', 'naive', a positive integer, or a pair of scalars.")
})
test_that("use splr", {
  expect_equivalent(as.matrix(center_graph(A, scheme = 1, use_splr = FALSE)),
                    matrix(c(rep(0, 6), 1, 0, 0),byrow = TRUE, nrow=3))
  expect_equivalent(as.matrix(center_graph(A, scheme = c(-1, 1), use_splr = FALSE)),
                    matrix(c(-1,-1,1,-1,-1,-1,1,-1,-1),nrow=3))
})

A <- sample_correlated_gnp_pair(100, .5, .5)$graph1
test_that("use splr", {
  expect_equivalent(nrow(center_graph(A, scheme = 2, use_splr = FALSE)), 100)
})


test_that("padding", {
  expect_equivalent(nrow(pad(A[], 2)), 102)
  expect_equivalent(nrow(pad(center_graph(A, scheme = 2, use_splr = TRUE), 2)), 102)
})
...
test_that("padding for multi-layer graphs", {
  expect_equivalent(nrow(pad(A, 2)), 22)
})
